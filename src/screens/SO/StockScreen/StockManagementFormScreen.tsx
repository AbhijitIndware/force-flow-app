...
const StockManagementFormScreen = ({ navigation, route }: Props) => {
    ...
    const { data: stockStatusData } = useGetStoreStockStatusQuery({ store }, { refetchOnMountOrArgChange: true });
    ...
    const handleRemoveItem = (index: number) => {
        setEntries(prev => {
            ...
            if (!next.some(entry => !entry.isPrev)) {
                return [...next, blankRow];
            }
            return next;
        });
    };
    ...
    useEffect(() => {
        if (stockStatusData?.message) {
            let all = [...stockStatusData.message.all_items, ...stockStatusData.message.previous_items]
            setAllItems(all);
        }
    }, [stockStatusData]);
    return (
                    ...
                    <ScrollView
                        ...
                        <View>
                            ...
                            {entries.map((entry, index) => {
                                ...
                                const matchItem = allItems?.find(item => item?.item_code === entry.itemCode)
